...
@pytest.fixture(scope="function")
def set_tmp_config(base_app):
    """Fixture to temporarily set config values.
    ...
    `set_tmp_config` provides a function to call to temporarily set config
    ...
    .. code-block:: python
        ...
        def test_with_tmp_config(app, set_tmp_config):
            set_tmp_config({
    ...
    for key, (key_present, value) in config_prev.items():
        if key_present:
            ...
            # in case the key was deleted during the test i.e., to be super
            # clean
